...
The `repository` schema is the base type for all repository entities regardless of hosting platform (GitHub, Azure DevOps, GitLab, etc.). It combines `entity.json` and `external.json`, meaning it carries a `urls` map with a canonical web URL. Platform-specific subtypes (e.g. `git-repository`, `github-repository`) extend this schema with hosting-specific fields.
...
This is an abstract base type. Tools should create platform-specific subtypes (e.g. `git-repository`) rather than raw `repository` entities.
...
- [git-repository.json](git-repository-schema.md) - Git-layer repository schema
