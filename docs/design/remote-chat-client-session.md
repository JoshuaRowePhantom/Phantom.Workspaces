...
| Copilot SDK built-in tools (shell, filesystem, …) | **Remote** profile (self-invoked inside the CLI) |
...
|---|---|
| `AgentExecutor` | Executor instance E — default for `mcp`, `function`, and any unknown tool `kind`. |
| `GuiLocal` | GUI / initiating machine G — for `workspace-gui` and `workspace-entity`. |
| `HostingInstance` | Hosting instance H (owner of the workspace agent session) — for `agent-session` / `workspace-agent-session`. |
...
- `workspace-gui` / `workspace-entity` → `GuiLocal`.
...
- Everything else — `mcp`, `function`, `filesystem`, `web_request`, `chat-history`,
  `github-cli-builtin-tools`, and unknown kinds — falls through to `AgentExecutor`.

## Trust-profile selection

`TrustProfile` (`Phantom.Workspaces.Llm.Core/Trust/TrustProfile.cs`):

- `LocalClientInstance = "."` — the source instance.
- `WildcardClientInstance = "*"` — matches any client instance during
  `AllowsClientInstance`; used by permissive base profiles.
- `HostingWorkspacesClientInstances` — the effective list of client instances this
  profile may run on. Populating it with a non-`"."` id opts the session into remote
  hosting.
- `DefaultExecutionTarget` — a `JsonElement?` connection descriptor used as the
...
(`Phantom.Workspaces.Data.Core/JsonSchemas/agent-session.json:24`) — the entity id of
...
It is **not** the source of truth for the current run — see
...
- `docs/design/github-copilot-provider-support.md` — `CopilotSdkChatClient` details.
- `docs/design/llm-trust-profile.md` — trust-profile entities and composition.
- `docs/design/session-context-tools.md` — the "host follows the current run"
  rule for `agent-session` / `current-session` context.
- `docs/design/reverse-tunnel-trust-execution.md` — reverse-tunnel transport.

## Related documentation entities

- `["documentation", "agent-options", "providers"]` — the `github-copilot` "Remote
  hosting" subsection.
- `["documentation", "agent-options", "tools"]` — the "Execution target of tool
  kinds" table.
- `["documentation", "agent-options", "parameters"]` — the `trust-profile`
  parameter reference.
- `["documentation", "agent-configuration"]` — remote-hosting worked example for
