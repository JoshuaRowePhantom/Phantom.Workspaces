...
/// Either <see cref="DocumentTabId"/> identifies an existing (or open-but-unselected) tab, or — for the
...
/// constructor, so call sites read as <c>new NavigationTarget { DocumentTabId = ..., WorkspaceTabId = ... }</c>.
/// The <c>DocumentTabId</c>/<c>WorkspaceTabId</c> names were renamed from <c>TabId</c>/<c>WorkspacePaneId</c>
/// for clarity while keeping identical semantics (tab id, pane id); see #1341.
/// </remarks>
public sealed record NavigationTarget
{
    ...
    public string? DocumentTabId { get; init; }

    /// <summary>An optional hint for the workspace pane that owns the tab (opened if not yet loaded).</summary>
    public string? WorkspaceTabId { get; init; }

    /// <summary>The entity to open when <see cref="DocumentTabId"/> is <see langword="null"/> (brain fallback).</summary>
